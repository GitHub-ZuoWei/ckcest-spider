...
class SpiderCkcest:
    ...
    def __init__(self):
        ...
        # 表
        self.ckcest_specialist_table = self.db['ckcest_specialist']
        self.ckcest_project_table = self.db1['ckcest_project']
        self.ckcest_thesis_table = self.db2['ckcest_thesis']
        ...
        self.ckcest_specialist_cookies = get_ckcest_specialist_cookies()
        ...
        self.tmp_cookies = {
            '_Collect_UD': 'c6965b97-ea20-42c2-b0a2-a92dc93105c5',
            '_Collect_ISNEW': '1637138583455',
            '_uab_collina': '163713858346178810758068',
            '_YS_userAccect': 'be824b83-81f1-4171-8eb9-5ef9e9044612',
            '_Collect_SN': '1',
            'Hm_lvt_789fd650fa0be6a2a064d019d890b87f': '1635917333,1636091311',
            'JSESSIONID': '3F91CB79249AEF834FF0E3ED5B1C7751',
            'acw_tc': '276aede916375684572988856e30e29efce2d0f968dba655335711e2ae0837',
            'acw_sc__v2': '619b4fc9c80cdcbcfbebbaa8e3ba6033c141c1c1',
            'Hm_lpvt_789fd650fa0be6a2a064d019d890b87f': '1637569194',
            'ssxmod_itna': 'YqGx2DBDcQD=0QA50LIO8D9lDyARDg7D/ACQc=Gx0vhReGzDAxn40iDt=ryGiQa7GoeCFO++x4/8xfidpeFhDm+ar5aDBeDHxY=DUZipxaD445GwD0eG+DD4DW3qAoDexGPynLMKGWD4qDRDAQDzMdVB0=YY06SS4DgKeG2DGU5fuDqCqDMD7tD/bxikeDB41W4RLtQcmIxBDDH3Y9mKYw8xYpaiL04xBQD7kiyDYEXIQ9B9dHG87P5IB5=IA3qzYxi8GG5RAxWeTxbYAxN0iY4BAhQQxeQm/1qDDfmUBkqeD===',
            'ssxmod_itna2': 'YqGx2DBDcQD=0QA50LIO8D9lDyARDg7D/ACQc=DnI3YrKDsKCqDLGmpaynDRxqAPkD=CdA=4KoFdE4exdIWqWIc+xlr6+N2FuKAwcR/q7w13uIpL+8CW8T10jjgIaH12XLrNCFqemMQRGq=X8wfjDkE64RQkKWdp7uIcRwd1+qY1BTuZWb51WCTpCdnK6SiheQi9WjYuYMc8DdE2ASt7f1FfG=yuzbyVlc9GxWy73RNDuRQmuM5kAWKdhoS1dS2EOQ3wmbLsb8oeQUIRtQC/4Ntnc+CuxXCkeF1LIULgdXYvaPj+S4/M7GEhGqEI9=0ZQ=yYQGQE9EDBads6mK7XjgNEgNpxwjnp7QmBBXzcru7DxtmzrXvnrC=eyD6RgIvBNO8N/A2q=mF=2zhT8atkh2LxWwBX+lupDN2E4DQ9qBrPVu=Uq5fPWp2FAfWtGQ9Srai8QiqIOr/uH5q4D08DiQ4YD===',
        }
    ...
    def ckcest_spider(self):
        ...
        for new_energy_keywords in open("../spider_keywords.txt", encoding="UTF-8"):
            ...
            request_data = {
                "dbId": "2002",
                "text": "",
                "express": f"AB={keywords_quote} OR OG={keywords_quote} OR KY={keywords_quote}",
                "secondSearchExpress": "",
                "order": "101",
                "startYear": '1700',
                "endYear": '2099',
                            ...
                            if x.keys() != 'showName':
                                ...
                                if ''.join(parm) in x:
                                    ...
                                    for num in range(1, 101):
                                        ...
                                        json_data = self.session.post(url, headers=self.header, data=request_data)
                                        try:
                                        ...
                                        for item in results_datas:
                                            ...
                                            while True:
                                                try:
                                                    # proxy_ip = self.proxy_utils.get_random_ip()
                                                    # print(proxy_ip)
                                                    achievement = self.session.get(
                                                        'http://www.ckcest.cn/default/es3/detail/1000/dw_expert_2020_merge/' + data_id,
                                                        # cookies=self.tmp_cookies, headers=header)
                                                        cookies=self.ckcest_cookies, headers=self.header,
                                                        # proxies={"http": proxy_ip}
                                                        proxies={"http": "192.168.17.243:10809"}
                                                        # proxies={"http": "192.168.12.180:6666"}
                                                    )
                                                    break
                                                except Exception as e:
                                                    time.sleep(1)
                                                    retry_count += 1
                                                    logger.warning(f'重试{retry_count}次')

                                            html_element = etree.HTML(achievement.text)
                                            try:
                                                content_element = html_element.xpath(
                                            ...
                                            if 'email' in item:
                                                del item['email']
                                            if 'become_year' in item:
                                                del item['become_year']
                                            ...
                                            del item['RD']
                                            # 个人简介
                                            item['summary'] = self.format_string_data(
                                                item['AB'] if 'AB' in item else '')
                                            if 'AB' in item:
                                                del item['AB']
                                            if 'expert_type' in item:
                                                if '院士' in item['expert_type']:
                                                    item['expert_type'] = '院士'
                                            # if 'gender' in item:
                                            ...
                                            if 'project' in expertResultStr:
                                                for expertResult in expertResultStr['project']:
                                                    ...
                                                    sponsor_strftime = datetime.strptime(
                                                        str(expertResult['publish_date']),
                                                        '%Y%m%d')
                                                    format_sponsor_time = datetime.strftime(sponsor_strftime,
                                                                                            '%Y-%m-%d')
                                                    ...
                                                    dict_data = {
                                                        ...
                                                        # 标题
                                                        "title": cc.convert(
                                                            ''.join(expertResult['TI']).replace('<em>', '').replace(
                                                                '</em>',
                                                        ...
                                                        "title_translate": ''.join(
                                                            expertResult['title_translate_en']).replace('<em>',
                                                                                                        '').replace(
                                                            '</em>',
                                                            '') if "title_translate_en" in expertResult else None,
                                                        # 出版年份
                                                        ...
                                                        "key_words_other": [
                                                            ...
                                                            for x in
                                                            expertResult['KY']] if "KY" in expertResult else None,
                                                        # 关键词
                                                        "key_words": [
                                                            cc.convert(
                                                                ''.join(x).replace('<em>', '').replace('</em>', ''))
                                                        ...
                                                        "zh_summary": cc.convert(
                                                            ''.join(expertResult['AB']).replace('<em>', '').replace(
                                                                '</em>',
                                                    ...
                                                    self.ckcest_project_table.insert(dict_data, check_keys=False)
                                            # 期刊论文
                                            if 'journal_article' in expertResultStr:
                                                for expertResult1 in expertResultStr['journal_article']:
                                                    ...
                                                    if self.is_parse(journal_article_id):
                                                        ...
                                                        continue
                                                    sponsor_strftime = datetime.strptime(
                                                        str(expertResult1['publish_date']),
                                                        '%Y%m%d')
                                                    format_sponsor_time = datetime.strftime(sponsor_strftime,
                                                                                            '%Y-%m-%d')
                                                    ...
                                                    related_dict_data = {
                                                        ...
                                                        # 标题 1
                                                        "title": cc.convert(
                                                            str(expertResult1['TI']).replace('<em>', '').replace(
                                                                '</em>',
                                                        ...
                                                        # 摘要
                                                        "summary": cc.convert(
                                                            ''.join(str(expertResult1['AB'])).replace('<em>',
                                                                                                      '').replace(
                                                                '</em>',
                                                    ...
                                                    self.ckcest_thesis_table.insert(related_dict_data, check_keys=False)
                                            # 合作学者
                                            if 'author' in expertResultStr:
                                                author_ = expertResultStr['author']
                                                item['related_author'] = author_
                                                # Todo
                                                for _ in author_:
                                                    self.redis_client.sadd('ckcest_person_id', _['id'])
                                            # 合作机构
                                            if 'organ' in expertResultStr:
                                                org_list = []
                                                for _ in expertResultStr['organ'].keys():
                                                    org_list.append(_)
                                                item['related_org'] = org_list
                                            # 作者关键词
                                            if 'keywords' in expertResultStr:
                                                keywords_list = []
                                                for _ in expertResultStr['keywords'].keys():
                                                    keywords_list.append(_)
                                                item['author_keywords'] = keywords_list
                                            # 关键词年份
                                            if 'keywords_year' in expertResultStr:
                                                item['author_keywords_year'] = expertResultStr['keywords_year']
                                            ...
                                            if 'picture_url' in item:
                                                ...
                                                if picture_url.startswith('http'):
                                                    ...
                                                    try:
                                                        ...
                                                        img_name = f"{data_id}&id=1.{suffix}"
                                                        urlretrieve(picture_url, filename=fr"D:\Resource\\{img_name}")
                                                        item['picture_name'] = img_name
                                                    except Exception as e:
                                                        logger.error('PICTURE DOWNLOAD FAILED URL: ')
                                                        logger.error(e)
                                                del item['picture_url']
                                            self.ckcest_specialist_table.insert(item, check_keys=False)
                                            self.insert_num += 1
    ...
    # 解析相关人物详情页
    def parse_person_info(self, person_id, item):

        achievement = self.session.get(
            'http://www.ckcest.cn/default/es3/detail/1000/dw_expert_2020_merge/' + person_id,
            # cookies=self.tmp_cookies, headers=header)
            cookies=self.ckcest_cookies, headers=self.header)
        html_element = etree.HTML(achievement.text)
        ...
        expertResultStr = {}
        for item_element in content_element:
            item_data = re.findall("var expertResultStr = '\s|[\r\n](.*?)';\s|[\r\n]        var md5 =",
                                   item_element.xpath('string(.)'))
            for content in item_data:
                unquote = urllib.parse.unquote(content)
                if unquote.startswith('        var expertResultStr = '):
        ...
        # item['person_detail'] = detailStr
        # item['category_professional'] = acadetit
        item['category_subject'] = ''  # 搜索条件的领域
        item['category_id'] = ''  # 八大领域ID
        ...
        if 'project' in expertResultStr:
            for expertResult in expertResultStr['project']:
                ...
                self.ckcest_project_table.insert_one(dict_data)
        # 期刊论文
        if 'journal_article' in expertResultStr:
            for expertResult1 in expertResultStr['journal_article']:
                journal_article_id = expertResult1['_id']
                ...
                try:
                    self.ckcest_thesis_table.insert_one(related_dict_data)
                except:
                    print(thesis_key_words)
        # 合作学者
        if 'author' in expertResultStr:
            item['related_author'] = expertResultStr['author']
        # 合作机构
        if 'organ' in expertResultStr:
            org_list = []
            for _ in expertResultStr['organ'].keys():
        ...
        if 'keywords' in expertResultStr:
            keywords_list = []
            for _ in expertResultStr['keywords'].keys():
                keywords_list.append(_.replace('.', '\u002E'))
            item['author_keywords'] = keywords_list
        # 关键词年份
        if 'keywords_year' in expertResultStr:
            item['author_keywords_year'] = expertResultStr['keywords_year']
        ...
        if 'picture_url' in item:
            ...
            del item['picture_url']
        self.ckcest_specialist_table.insert_one(item)
        self.insert_num += 1
        logger.info(f'INSERT NUM : {self.insert_num}')
