...
class SpiderCkcest:
    ...
    def ckcest_spider(self):
        ...
        # for acadetit in ['研究员', '教授', '副教授', '副研究员', '副主任医师', '高级工程师']:
        # 学科领域 RD
        for rd in ['计算机科学技术', '体育科学', '国民经济学', '化学工程', '工程与技术科学基础学科', '教育学', '化学', '电子与通信技术', '经济学', '基础医学',
                   '土木建筑工程', '预防医学与公共卫生学', '工业经济学', '临床医学', '电气工程']:

            enum_id = None
            for enum_item in ZJENUM:
                if rd in ZJENUM[enum_item]['value']:
                    enum_id = ZJENUM[enum_item]['id']

            for num in range(1, 101):
                # url = f'http://mall.ckcest.cn/mall/listContent.ilf?dbId=2003&text=&express=&secondSearchExpress=ACADETIT%3D%3D{urllib.parse.quote(acadetit)}+AND+RD%3D%3D{urllib.parse.quote(rd)}+AND+OG%3D{urllib.parse.quote("军")}&secondSearchFlag=0&fieldSearchExpress=&fieldSearchEntity=&order=1&startYear=1700&endYear=2099&page={num}&limit=10&model=1&isadvanced=0&picture=&uuid='
                url = f'http://mall.ckcest.cn/mall/listContent.ilf?dbId=2003&text=&express=&secondSearchExpress=RD%3D%3D{urllib.parse.quote(rd)}+AND+OG%3D{urllib.parse.quote("军")}&secondSearchFlag=0&fieldSearchExpress=&fieldSearchEntity=&order=1&startYear=1700&endYear=2099&page={num}&limit=10&model=1&isadvanced=0&picture=&uuid='
                json_data = self.session.get(url, headers=self.header, cookies=self.ckcest_specialist_cookies)
                try:
                ...
                for item in results_datas:
                    ...
                    item['category_subject'] = rd  # 搜索条件的领域
